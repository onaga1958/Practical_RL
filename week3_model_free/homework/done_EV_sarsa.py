...
class EVSarsaAgent():
    ...
    """
    def __init__(self, alpha, discount, getLegalActions, softmax=False, epsilon=0.25, tau=1):
    ...
    def _softmax(self, values):
        scaled_values = values - np.max(values)
        values_exp = np.exp(scaled_values / self.tau)
        probas = values_exp / np.sum(values_exp)
        return probas

    def getValue(self, state):
        """
        Returns V(s) according to expected value SARSA algorithm
        This should be equal to expected action q-value over action
        probabilities defined by epsilon-greedy policy with current epsilon.
        """
        ...
        if self.softmax:
            ...
            probas = self._softmax(values)
            return np.sum(probas * values)
        else:
            return (
                np.mean([self.getQValue(state, action) for action in possibleActions]) * epsilon +
    ...
    def getAction(self, state):
        ...
        epsilon = self.epsilon
        if self.softmax:
            action = np.random.choice(possibleActions, p=self.getPolicy(state))
        else:
    ...
    def update(self, state, action, nextState, reward):
        ...
        updated_qvalue = (
            (1-learning_rate) * self.getQValue(state, action) + learning_rate * reference_qvalue
        )
