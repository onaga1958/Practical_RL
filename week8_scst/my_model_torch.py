...
class DoubleTranslationModel(nn.Module):
    def __init__(self, inp_voc, out_voc,
        ...
        self.enc1 = nn.GRU(hid_size, hid_size, batch_first=True)
        self.dec_start = nn.Linear(hid_size, hid_size)
        self.dec_start_2 = nn.Linear(hid_size, hid_size)
    ...
    def encode(self, inp, **flags):
        ...
        enc_last_2 = enc_seq_2[range(0, enc_seq_2.shape[0]), end_index.detach(), :]

        dec_start = self.dec_start(enc_last)
        dec_start_2 = self.dec_start_2(enc_last_2)
        return [dec_start, dec_start_2]
    ...
    def decode(self, prev_state, prev_tokens, **flags):
        ...
        """
        prev_dec, prev_dec_2 = prev_state
        prev_emb = self.emb_out(prev_tokens)
        new_dec_state = self.dec0(prev_emb, prev_dec)
        new_dec_state_2 = self.dec1(new_dec_state, prev_dec_2)
        output_logits = self.logits(new_dec_state_2)

        return [new_dec_state, new_dec_state_2], output_logits
